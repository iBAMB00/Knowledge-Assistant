"""Request-level Agent trace composition for v2.5-A3 production wiring."""
...
class AgentRunTraceSession:
    """One active provider-backed Agent trace and its model-call tracer."""
    ...
    def finish(self, *, ok: bool = True, error_code: str | None = None) -> None:
        try:
            ...
            logger.warning(
                "Agent observability finish failed; ignoring provider error",
                exc_info=True,
            )
...
def start_agent_run_trace(
    *,
    ...
    """Start tracing only when the configured provider is active."""
    ...
    try:
        trace_handle = provider.start_trace(
            trace_context=trace_context,
            name="agent.run",
        )
    except Exception:
        logger.warning(
            "Agent observability start failed; continuing without trace",
            exc_info=True,
        )
        return None
