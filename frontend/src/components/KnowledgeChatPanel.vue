...
import { Database, Send, Square } from "lucide-vue-next";
...
<template>
  <div class="chat-layout">
    <aside class="chat-context-panel">
      <div class="mode-context-heading">
        <strong>知识问答</strong>
        <span>Deterministic RAG Baseline</span>
      </div>
      ...
      <label class="stream-setting">
        <div><strong>流式回答</strong><span>使用 SSE 实时显示 RAG 模型输出</span></div>
        <input type="checkbox" :checked="streamingEnabled" @change="onStreamingChange" />
      </label>

      <div class="context-note">
        <strong>稳定检索基线</strong>
        <p>请求继续走 /knowledge/chat；知识库范围由后端 RBAC 与检索过滤共同限制。</p>
      </div>
    </aside>
    ...
    <main class="chat-surface">
      ...
      <div class="chat-composer-wrap">
        <textarea v-model="question" rows="3" maxlength="2000" :disabled="submitting || !selectedKnowledgeBaseId" :placeholder="selectedKnowledgeBaseId ? '输入知识问题，Enter 发送，Shift + Enter 换行' : '请先选择一个知识库'" @keydown="onKeydown" />
